# QuantNifty

Landing page for QuantNifty , an algorithmic trading system development and consulting firm
for Indian index derivatives.

This website is the `Website/` subproject in the QuantNifty workspace. Run all
`npm` and `vercel` commands from this directory. For Git deployments, configure
the Vercel project's Root Directory as `Website`.

## What's here

A single self-contained `index.html` (no build step, no dependencies). Fonts load from Google Fonts;
everything else , SVG charts, styles, interactions , is inline.

## Sections

- **Hero** , interactive Alpha equity curve with hover crosshair and tooltip
- **The Double Engine** , stacking calculator: equity CAGR + engine return on pledged margin
- **Engines** , Apex (NIFTY), Alpha (NIFTY+SENSEX), Whale (drawdown-capped)
- **Sharpe** , benchmark comparison
- **Risk dial** , interactive drawdown-ceiling sizing
- **Contact** , enquiry form routed to WhatsApp

## Before going live

- [ ] Replace the WhatsApp placeholder `919999999999` (2 occurrences in `index.html`)
- [ ] Confirm the performance figures and disclosure wording

## Disclosures

QuantNifty is **not** a SEBI-registered Research Analyst, Investment Adviser, Portfolio Manager or
Broker. Nothing on the site is investment advice. All performance figures shown are **backtested**,
not a live track record. Full disclosures are in the site footer.
